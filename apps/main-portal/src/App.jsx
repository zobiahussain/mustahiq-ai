...
export default function App() {
  ...
  return <div className="app"><a href="#main-content" className="skip-link">Skip to content</a><div className="utility"><span>Alkhidmat Foundation Pakistan <i/> Serving humanity since 1990</span><span>{data.demo_mode ? 'SYNTHETIC RECORDS' : 'AUTHORIZED STAFF WORKSPACE'}</span></div><header className="topbar"><div className="brand"><button className="icon-button mobile-only" aria-label="Toggle navigation" onClick={() => setMenu(!menu)}><Menu/></button><img src="/alkhidmat-logo.svg" alt="Alkhidmat Foundation Pakistan"/><span className="brand-divider"/><div className="product-name">Mustahiq<span>AI</span><small>BENEFICIARY CARE PLATFORM</small></div></div><div className="topbar-right"><span className="workspace-tag"><span className="status-dot"/>Staff workspace</span><div className="staff-mini"><Avatar name={data.staff.full_name} small/><span><strong>{data.staff.full_name}</strong><small>{titleCase(data.staff.role)}</small></span></div></div></header>
    ...
    <main id="main-content" className="main-content"><div className="breadcrumb"><span>Staff portal</span><span>/</span><strong>{navigation.find(n => n[0] === route)?.[1]}</strong><button className="icon-button" disabled={pending} aria-label="Refresh workspace" onClick={() => load().catch(() => {})}><RefreshCw size={16}/></button></div>{data.demo_mode && <div className="demo-strip"><ShieldCheck size={14}/><span>These records are synthetic — no real beneficiary data. Programmes, budgets and policies are illustrative.</span></div>}{error && <Alert type="error">{error}</Alert>}<Page {...shared}/><footer className="page-footer"><span>Alkhidmat Foundation Pakistan · Mustahiq AI</span><span>AI-assisted discovery. Human-led decisions.</span></footer></main></div>
