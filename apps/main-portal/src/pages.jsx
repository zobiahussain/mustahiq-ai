...
import { ArrowRight, ArrowUpRight, Users, Sparkles, ClipboardCheck, HeartHandshake, Plus, Search, Filter, Check, X, FileCheck, Clock, ShieldCheck, Download, Info } from 'lucide-react';
...
export function Dashboard({ data, navigate, open }) {
  ...
  const metrics = [
    ...
    ['Suggestions to review', pending.length, Sparkles, 'Potential support, awaiting your review', 'discovery'],
    ['Awaiting outreach', outreach.length, ClipboardCheck, 'Ready for a conversation', 'outreach'],
    ['Verified candidates', verified.length, ShieldCheck, 'In the shared candidate pool', 'candidates'],
  ];
  return <><PageHeading eyebrow="TOGETHER, WE MAKE A DIFFERENCE" title={`Welcome back, ${data.staff.full_name.split(' ')[0]}.`} description="A little clarity. A lot more care. Here’s where your team can help today."><span className="date-label">{dateLabel(new Date())}</span><button className="button primary" onClick={() => open({ type: 'profile-form' })}><Plus size={17}/> Register beneficiary</button></PageHeading>
    <section className="welcome-banner"><div><span className="eyebrow">FROM NEED TO OPPORTUNITY</span><h2>The right support.<br/>For the people who need it.</h2><p>Bring every case into view. Find potential support,<br className="desktop-only"/> verify with care, and make every allocation count.</p><button onClick={() => navigate('discovery')}>Review potential matches <ArrowRight size={17}/></button></div><div className="hero-journey"><div className="journey-orbit"/><div className="journey-seal"><HeartHandshake size={42}/><strong>Humanity<br/>at the heart.</strong></div><span className="orbit-label orbit-one"><Sparkles size={15}/> Discover</span><span className="orbit-label orbit-two"><ClipboardCheck size={15}/> Verify</span><span className="orbit-label orbit-three"><HeartHandshake size={15}/> Support</span></div></section>
    ...
    <div className="dashboard-grid"><Panel title="Your next steps" subtitle="Small actions that move someone closer to support." action={<LinkButton onClick={() => navigate('discovery')}>View worklist</LinkButton>}>
      <div className="action-list">{[[Sparkles, 'Review discovered opportunities', `${pending.length} suggestions need a staff decision`, 'discovery', pending.length], [ClipboardCheck, 'Reach out and verify needs', `${outreach.length} cases are ready for assessment`, 'outreach', outreach.length], [FileCheck, 'Review the candidate pool', `${verified.length} verified candidates across programs`, 'candidates', verified.length]].map(([Icon, title, sub, target, count]) => <button key={title} className="action-row" onClick={() => navigate(target)}><span className="action-icon"><Icon size={21}/></span><span><strong>{title}</strong><small>{sub}</small></span><span className="count-chip">{count}</span><ArrowRight size={17}/></button>)}</div>
      <div className="panel-note"><ShieldCheck size={16}/> Discovery is a suggestion. Verification comes before allocation.</div></Panel>
      <Panel title="The care journey" subtitle="One fair process. Every beneficiary."><div className="care-journey">{[['Registered', data.profiles.length], ['Awaiting review', pending.length], ['In outreach', outreach.length], ['Verified pool', verified.length], ['Supported', disbursed.length]].map(([label, count], i) => <div key={label}><span className={`step-number step-${i}`}>{i + 1}</span><span>{label}</span><strong>{count}</strong></div>)}</div></Panel>
    </div>
    <Panel title="Recently registered" subtitle="People behind the profiles." action={<LinkButton onClick={() => navigate('beneficiaries')}>All beneficiaries</LinkButton>}><Table rows={[...data.profiles].sort((a, b) => b.created_at.localeCompare(a.created_at)).slice(0, 5)} columns={[
...
export function Outreach({ data, open }) {
  ...
  return <><PageHeading title="Conversations that confirm care." eyebrow="OUTREACH & VERIFICATION" description="Confirm real need, check other assistance, and record what you learn."/><section className="panel"><div className="tabs"><button className={!history ? 'selected' : ''} onClick={() => setHistory(false)}>Needs verification <span>{combined.length}</span></button><button className={history ? 'selected' : ''} onClick={() => setHistory(true)}>Verification history <span>{data.verifications.length}</span></button></div><div className="toolbar"><SearchBox value={search} onChange={setSearch}/><ProgramFilter data={data} value={program} onChange={setProgram}/></div><Table rows={rows} empty={history ? 'No assessments recorded yet' : 'No cases awaiting verification'} columns={[
...
export function Candidates({ data, open, mutate, pending }) {
  ...
  return <><PageHeading eyebrow="ONE POOL. IDENTICAL CRITERIA." title="Prioritize need. Allocate with care." description="Verified candidates are ranked using a transparent, program-specific rubric."><button className="button secondary" onClick={() => setShowPolicy(!showPolicy)}><Info size={16}/> {showPolicy ? 'Hide rubric' : 'View rubric'}</button></PageHeading>
    ...
    {confirmRank && <Modal title="Run a new ranking cycle" onClose={() => !pending && setConfirmRank(false)}><p>Rank verified candidates for <strong>{program.name}</strong> using the current rubric. Expired verifications return to outreach. Scores and weights are saved for audit.</p><Alert>Review the rubric first: its normalization caps are demonstration policy and need your department’s agreement.</Alert>{error && <Alert type="error">{error}</Alert>}<div className="form-actions"><button className="button secondary" onClick={() => setConfirmRank(false)}>Cancel</button><button className="button primary" disabled={pending} onClick={run}>{pending ? 'Ranking…' : 'Run ranking'}</button></div></Modal>}
...
export function Reports({ data }) {
  ...
  return <><PageHeading eyebrow="DEPARTMENT INSIGHTS" title="See the support taking shape." description="A clear view of discovery, verification, and recorded allocations across your programs."><button className="button secondary" onClick={download}><Download size={17}/> Export summary</button></PageHeading><div className="report-banner"><HeartHandshake size={36}/><div><strong>{data.applications.filter(a => a.status === 'disbursed').length}</strong><span>allocations recorded</span></div><div><strong>{money(summary.reduce((sum, p) => sum + p.amount, 0))}</strong><span>support recorded across programs</span></div><p>Every number starts<br/>with someone’s story.</p></div><Panel title="Program overview" subtitle="Counts reflect the current accessible workspace, not organization-wide impact."><Table rows={summary} columns={[
